...
void Renderer::createBuffers() {
  // ---- Curve buffer (HOST_VISIBLE | HOST_COHERENT, persistently mapped) ----
...
void Renderer::dispatch(VkCommandBuffer cmd) {
  ...
  uint32_t pushData[4] = {screenWidth, screenHeight, curveCount, tileCountX};

  vkCmdBindDescriptorSets(cmd, VK_PIPELINE_BIND_POINT_COMPUTE,
                          computePipelineLayout, 0, 1, &descriptorSet, 0,
                          nullptr);

  // The tile buffer's count slots must be zeroed each frame so InterlockedAdd
  // starts from a clean state. vkCmdFillBuffer is the cheap way (transfer op).
  const VkDeviceSize tileBufferSize = static_cast<VkDeviceSize>(tileCountX) *
                                      tileCountY * TILE_STRIDE_U32 *
                                      sizeof(uint32_t);
  vkCmdFillBuffer(cmd, tileBuffer, 0, tileBufferSize, 0);

  // Make the fill visible to the tiling shader before it reads/writes counts.
  {
  ...
  if (curveCount > 0) {
    ...
    // Tile buffer write -> read between the two compute passes.
    VkBufferMemoryBarrier bb{};
    bb.sType               = VK_STRUCTURE_TYPE_BUFFER_MEMORY_BARRIER;
